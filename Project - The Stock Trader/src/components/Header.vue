<template lang="html">
  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
      <ul class="nav navbar-nav" v-if="this.$store.state.logged">
        <router-link to="/portfolio" activeClass="active" tag="li"><a>Portfolio</a></router-link>
        <router-link to="/stocks" activeClass="active" tag="li"><a>Stocks</a></router-link>

      </ul>
      <strong class="navbar-text navbar-right">Funds: {{ funds | currency }}</strong>
      <ul class="nav navbar-nav navbar-right" v-if="!this.$store.state.logged">
        <li><a @click="login" class="pointer">Log in</a></li>
      </ul>
      <ul class="nav navbar-nav navbar-right" v-else>
        ...
        <li><a @click="logout" class="pointer">Log out</a></li>
      </ul>
    </div><!-- /.navbar-collapse -->
  </div><!-- /.container-fluid -->
</nav>
